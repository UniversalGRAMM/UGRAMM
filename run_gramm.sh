...
Arch=RIKEN_with_pins #set to RIKEN for generating without pins

# Generating device model:
echo "---------------------Generating device model using external script:---------------------"
echo "cd scripts && ./device_model_gen.py -NR $2 -NC $3 -Arch RIKEN_with_pins && cd .."
cd scripts && ./device_model_gen.py -NR $2 -NC $3 -Arch RIKEN_with_pins && cd ..
